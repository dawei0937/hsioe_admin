...
def deleteMsg(redis,msgId,memberId):
    ...
    从玩家列表中移除
    """
    log_debug('[FUNC][deleteMsg][info] msgId[%s] memberId[%s]'%(msgId,memberId))
    ...
    log_debug('[FUNC][deleteMsg][error] msgId[%s] memberId[%s] 1111'%(msgId,memberId))
    pipe.execute()
...
def createNotice(redis,agentId,messageInfo):
    """
    创建一条新信息
    """
    ...
    readUrl = '/admin/notice/read?type=%s&id=%s&agentId=%s'%(messageInfo['messageType'],noticId,agentId)
    messageInfo['read'] = '0'
...
def getNoticsList(redis,session,lang,agentId):
    """
    获取已发送公告列表
    """
    if int(agentId) == systemId:
        noticIds = redis.lrange(FORMAT_GAMEHALL_NOTIC_LIST_TABLE,0,-1)
    else:
        noticIds = redis.lrange(FORMAT_MGR_SEND_MESSAGE_LIST%(agentId),0,-1)

    log_debug('[FUNC][getNoticsList][info] agentId[%s] noticIds[%s]'%(agentId,noticIds))
    ...
    for noticId in noticIds:
        ...
        log_debug('[FUNC][getNoticsList][info] agentId[%s] noticIds[%s] status[%s]'%(agentId,noticIds,noticeInfo['status']))
        noticeInfo['op'] = []
        # sessiob['access']
