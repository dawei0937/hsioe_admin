...
def getAgentMembers(redis,agentId):
    ...
    for subId in agentIdChilds:
        ...
        log_debug('[agentModel][Func][getAgentAllMembers] agentId[%s] downLineCount[%s]'%(subId,downLineCount))
...
def getAgentAllMembers(redis,agentId):
    ...
    log_debug('[agentModel][Func][getAgentAllMembers] agentId[%s] agentIdChilds[%s]'%(agentId,agentIdChilds))
...
def getAgentActiveReport(redis,agentId,startDate,endDate):
    """
    获取某个代理下的会员活跃数
    """
    parentTable = AGENT_CHILD_TABLE%(agentId)
    ...
    log_debug('[getAgentActiveReport][FUNC] agentId[%s] subAgentIds[%s]'%(agentId,agentIds))
    ...
    while endDate >= startDate:
        for agent_id in agentIds:
            ...
            agentInfo['account'] = agentDetail['account']
            count = redis.get(DAY_AG_LOGIN_COUNT%(agent_id,agentInfo['date']))
            if not count:
                count = 0
            count = int(count)
            parentTable = AGENT_CHILD_TABLE%(agent_id)
            agent_ids = redis.smembers(parentTable)
            count+=getDateAgTotal(redis,agent_id,agent_ids,agentInfo['date'],startDate)
